...
def ReadFromNc(filePath):
    ...
    #最小相对湿度
    total_cloud_cover_cloud_base = (nc_obj.variables['Total_cloud_cover_cloud_base'][:])
    ...
    ncdatadict['total_cloud_cover_cloud_base'] = total_cloud_cover_cloud_base
    return ncdatadict
...
def NcToJson(ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
        ...
        try:
            ...
            total_cloud_cover_cloud_base = ncdatadict['total_cloud_cover_cloud_base']
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        ...
                        lat = Lat[j]
                        lon = Lon[k]
                        cloud = total_cloud_cover_cloud_base[i, j, k]
                        attr = Attr[i, j, k]
                        ...
                        NcOneObjectDict['cloud'] = round(float(cloud), 6)
                        NcOneObjectDict[AttrName] = 0
                        #NcOneObjectDict[AttrName] = attr
                        jsondictList.append(NcOneObjectDict)

                        # #=============================连接数据库==================================
                        sql1 = "INSERT INTO tb_gdfs_cloud(gdfs_cloud_time,gdfs_cloud_lat,gdfs_cloud_lon,gdfs_cloud_value,gdfs_cloud_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,cloud,jrsjid1)
                        sql2 = "INSERT INTO tb_gdfs_cloud(gdfs_cloud_time,gdfs_cloud_lat,gdfs_cloud_lon,gdfs_cloud_value,gdfs_cloud_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,cloud,jrsjid2)
                        sql3 = "INSERT INTO tb_gdfs_cloud(gdfs_cloud_time,gdfs_cloud_lat,gdfs_cloud_lon,gdfs_cloud_value,gdfs_cloud_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,cloud,jrsjid3)
                        
                        try:
                            cursor1.execute(sql1)
                            print('tb_gdfs_cloud存入成功')
                            print('111111111111111111111111111111111111111111111111111111111111111111111111111')
                            # conn1.commit()
                        except Exception as e:
                            print("GDFSNcToJson_OEFS_ETC(1) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_OEFS_ETC(2) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_OEFS_ETC(3) ----- ",e)
                        
            try:
                cursor1.execute(sql4)
                # conn1.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_ETC(4) ----- ")
                
            try:
                cursor2.execute(sql5)
                # conn2.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_ETC(5) ----- ")
                
            try:
                cursor3.execute(sql6)
                # conn3.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_ETC(6) ----- ")

            return jsondictList

        except Exception as e:
            print("GDFSNcToJson_OEFS_ETC 文件解析异常 ----- ")

    except Exception as e:
        print("GDFSNcToJson_OEFS_ETC 数据库连接异常 ----- ")
    finally:
        cursor1.close()
        conn1.close()
        cursor2.close()
