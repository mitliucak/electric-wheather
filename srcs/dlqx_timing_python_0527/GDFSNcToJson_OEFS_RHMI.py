...
def ReadFromNc(filePath):
    ...
    #最小相对湿度
    height_above_ground_24_Hour_Minimum = (nc_obj.variables['VAR0-1-121_FROM_38-0--1_height_above_ground_24_Hour_Minimum'][:])
    ...
    ncdatadict['height_above_ground_24_Hour_Minimum'] = height_above_ground_24_Hour_Minimum
    return ncdatadict
...
def NcToJson(ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
        ...
        try:
            ...
            height_above_ground_24_Hour_Minimum = ncdatadict['height_above_ground_24_Hour_Minimum']
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        ...
                        lat = Lat[j]
                        lon = Lon[k]
                        minimum = height_above_ground_24_Hour_Minimum[i, 0, j, k]
                        attr = Attr[i, 0, j, k]
                        ...
                        NcOneObjectDict['humidity_min'] = round(float(minimum), 6)
                        NcOneObjectDict[AttrName] = 0
                        #NcOneObjectDict[AttrName] = attr
                        jsondictList.append(NcOneObjectDict)

                        # #=============================连接数据库==================================
                        sql1 = "INSERT INTO tb_gdfs_humidity_min(gdfs_humidity_min_time,gdfs_humidity_min_lat,gdfs_humidity_min_lon,gdfs_humidity_min_value,gdfs_humidity_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum,jrsjid1)
                        sql2 = "INSERT INTO tb_gdfs_humidity_min(gdfs_humidity_min_time,gdfs_humidity_min_lat,gdfs_humidity_min_lon,gdfs_humidity_min_value,gdfs_humidity_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum,jrsjid2)
                        sql3 = "INSERT INTO tb_gdfs_humidity_min(gdfs_humidity_min_time,gdfs_humidity_min_lat,gdfs_humidity_min_lon,gdfs_humidity_min_value,gdfs_humidity_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum,jrsjid3)

                        try:
                            cursor1.execute(sql1)
                            print('tb_gdfs_humidity_min存入成功')
                            print('111111111111111111111111111111111111111111111111111111111111111111111111111')
                            # conn1.commit()
                        except Exception as e:
                            print("GDFSNcToJson_OEFS_RHMI(1) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_OEFS_RHMI(2) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_OEFS_RHMI(3) ----- ",e)

            try:
                cursor1.execute(sql4)
                # conn1.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_RHMI(4) ----- ")
                
            try:
                cursor2.execute(sql5)
                # conn2.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_RHMI(5) ----- ")
                
            try:
                cursor3.execute(sql6)
                # conn3.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_RHMI(6) ----- ")

            return jsondictList

        except Exception as e:
            print("GDFSNcToJson_OEFS_RHMI 文件解析异常 ----- ")

    except Exception as e:
        print("GDFSNcToJson_OEFS_RHMI 数据库连接异常 ----- ")
    finally:
        cursor1.close()
        conn1.close()
        cursor2.close()
