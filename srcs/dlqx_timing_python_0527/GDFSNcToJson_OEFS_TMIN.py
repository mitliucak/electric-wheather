...
def ReadFromNc(filePath):
    ...
    #最低温度
    Minimum_temperature_height_above_ground_24_Hour_Minimum = (nc_obj.variables['Minimum_temperature_height_above_ground_24_Hour_Minimum'][:])
    ...
    ncdatadict['Minimum_temperature_height_above_ground_24_Hour_Minimum'] = Minimum_temperature_height_above_ground_24_Hour_Minimum
    return ncdatadict
...
def NcToJson(ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
        ...
        try:
            ...
            Minimum_temperature_height_above_ground_24_Hour_Minimum = ncdatadict['Minimum_temperature_height_above_ground_24_Hour_Minimum']
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        ...
                        lat = Lat[j]
                        lon = Lon[k]
                        minimum_temperature = Minimum_temperature_height_above_ground_24_Hour_Minimum[i, 0, j, k]
                        attr = Attr[i, 0, j, k]
                        ...
                        NcOneObjectDict['temperature_min'] = round(float(minimum_temperature), 6)
                        NcOneObjectDict[AttrName] = 0
                        #NcOneObjectDict[AttrName] = attr
                        jsondictList.append(NcOneObjectDict)

                        # #=============================连接数据库==================================
                        sql1 = "INSERT INTO tb_gdfs_temperature_min(gdfs_temperature_min_time,gdfs_temperature_min_lat,gdfs_temperature_min_lon,gdfs_temperature_min_value,gdfs_temperature_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum_temperature,jrsjid1)
                        sql2 = "INSERT INTO tb_gdfs_temperature_min(gdfs_temperature_min_time,gdfs_temperature_min_lat,gdfs_temperature_min_lon,gdfs_temperature_min_value,gdfs_temperature_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum_temperature,jrsjid2)
                        sql3 = "INSERT INTO tb_gdfs_temperature_min(gdfs_temperature_min_time,gdfs_temperature_min_lat,gdfs_temperature_min_lon,gdfs_temperature_min_value,gdfs_temperature_min_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,minimum_temperature,jrsjid3)

                        try:
                            cursor1.execute(sql1)
                            print('tb_gdfs_temperature_min存入成功')
                            print('111111111111111111111111111111111111111111111111111111111111111111111111111')
                            # conn1.commit()
                        except Exception as e:
                            print("GDFSNcToJson_OEFS_TMIN(1) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_OEFS_TMIN(2) ----- ",e)

                        try:
                            cursor3.execute(sql3)
                            print('tb_gdfs_temperature_man存入成功')
                            print('33333333333333333333333333333333333333333333333333333333333333333333333333333333333333333')
                            # conn3.commit()
                        except Exception as e:
                            print("GDFSNcToJson_OEFS_TMIN(3) ----- ",e)
            
            try:
                cursor1.execute(sql4)
                # conn1.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_TMIN(4) ----- ")
                
            try:
                cursor2.execute(sql5)
                # conn2.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_TMIN(5) ----- ")
                
            try:
                cursor3.execute(sql6)
                # conn3.commit()
            except Exception as e:
                print("GDFSNcToJson_OEFS_TMIN(6) ----- ")

            return jsondictList

        except Exception as e:
            print("GDFSNcToJson_OEFS_TMIN 文件解析异常 ----- ")

    except Exception as e:
        print("GDFSNcToJson_OEFS_TMIN 数据库连接异常 ----- ")
    finally:
        cursor1.close()
        conn1.close()
        cursor2.close()
