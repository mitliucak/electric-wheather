...
def ReadFromNc(filePath):
    ...
    #降水相态
    Precipitation_type_surface_3_Hour_Accumulation = (nc_obj.variables['Precipitation_type_surface_3_Hour_Accumulation'][:])
    ...
    ncdatadict['lat'] = lat
    ncdatadict['lon'] = lon
    ncdatadict['time'] = times
    ncdatadict['Attr'] = Attr
    ncdatadict['sinceTime'] = sinceTime
    ncdatadict['Precipitation_type_surface_3_Hour_Accumulation'] = Precipitation_type_surface_3_Hour_Accumulation
    return ncdatadict
...
def NcToJson(ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
        ...
        try:
            Lat = ncdatadict['lat']
            Lon = ncdatadict['lon']
            Times = ncdatadict['time']
            Attr = ncdatadict['Attr']
            sinceTime = ncdatadict['sinceTime']
            Precipitation_type_surface_3_Hour_Accumulation = ncdatadict['Precipitation_type_surface_3_Hour_Accumulation']
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        time1 = (sinceTime + datetime.timedelta(hours=Times[i])).strftime("%Y-%m-%d %H:%M:%S")
                        #time = Time[i]
                        lat = Lat[j]
                        lon = Lon[k]
                        precipitation_type = Precipitation_type_surface_3_Hour_Accumulation[i, j, k]
                        attr = Attr[i, j, k]

                        NcOneObjectDict = dict()
                        NcOneObjectDict['time'] = time1
                        NcOneObjectDict['lat'] = round(float(lat), 6)
                        NcOneObjectDict['lon'] = round(float(lon), 6)
                        NcOneObjectDict['precipitation_type'] = round(float(precipitation_type), 6)
                        NcOneObjectDict[AttrName] = 0
                        #NcOneObjectDict[AttrName] = attr
                        jsondictList.append(NcOneObjectDict)

                        # #=============================连接数据库==================================
                        sql1 = "INSERT INTO tb_gdfs_precipitation_type(gdfs_precipitation_type_time,gdfs_precipitation_type_lat,gdfs_precipitation_type_lon,gdfs_precipitation_type_value,gdfs_precipitation_type_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time1,lat,lon,precipitation_type,jrsjid1)
                        sql2 = "INSERT INTO tb_gdfs_precipitation_type(gdfs_precipitation_type_time,gdfs_precipitation_type_lat,gdfs_precipitation_type_lon,gdfs_precipitation_type_value,gdfs_precipitation_type_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time1,lat,lon,precipitation_type,jrsjid2)
                        sql3 = "INSERT INTO tb_gdfs_precipitation_type(gdfs_precipitation_type_time,gdfs_precipitation_type_lat,gdfs_precipitation_type_lon,gdfs_precipitation_type_value,gdfs_precipitation_type_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time1,lat,lon,precipitation_type,jrsjid3)

                        try:
                            cursor1.execute(sql1)
                            print('tb_gdfs_precipitation_type存入成功')
                            print('111111111111111111111111111111111111111111111111111111111111111111111111111')
                            time.sleep(0.1)
                            # conn1.commit()
                        except Exception as e:
                            print("GDFSNcToJson_QPF_PPH(1) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_QPF_PPH(2) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_QPF_PPH(3) ----- ",e)
            
            try:
                cursor1.execute(sql4)
                # conn1.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_PPH(4) ----- ")
                
            try:
                cursor2.execute(sql5)
                # conn2.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_PPH(5) ----- ")
                
            try:
                cursor3.execute(sql6)
                # conn3.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_PPH(6) ----- ")

            return jsondictList

        except Exception as e:
            print("GDFSNcToJson_QPF_PPH 文件解析异常 ----- ")

    except Exception as e:
        print("GDFSNcToJson_QPF_PPH 数据库连接异常 ----- ")
    finally:
        cursor1.close()
        conn1.close()
        cursor2.close()
