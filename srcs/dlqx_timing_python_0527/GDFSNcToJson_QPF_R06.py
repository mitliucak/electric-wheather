...
def ReadFromNc(filePath):
    ...
    #6小时降水
    Total_precipitation_surface_6_Hour_Accumulation = (nc_obj.variables['Total_precipitation_surface_6_Hour_Accumulation'][:])
    ...
    ncdatadict['Total_precipitation_surface_6_Hour_Accumulation'] = Total_precipitation_surface_6_Hour_Accumulation
    return ncdatadict
...
def NcToJson(ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
        ...
        try:
            ...
            Total_precipitation_surface_6_Hour_Accumulation = ncdatadict['Total_precipitation_surface_6_Hour_Accumulation']
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        ...
                        lat = Lat[j]
                        lon = Lon[k]
                        precipitation_6_Hour = Total_precipitation_surface_6_Hour_Accumulation[i, j, k]
                        attr = Attr[i, j, k]
                        ...
                        NcOneObjectDict['precipitation_6_Hour'] = round(float(precipitation_6_Hour), 6)
                        NcOneObjectDict[AttrName] = 0
                        #NcOneObjectDict[AttrName] = attr
                        jsondictList.append(NcOneObjectDict)

                        # #=============================连接数据库==================================
                        sql1 = "INSERT INTO tb_gdfs_precipitation_6_hour(gdfs_precipitation_6_Hour_time,gdfs_precipitation_6_Hour_lat,gdfs_precipitation_6_Hour_lon,gdfs_precipitation_6_Hour_value,gdfs_precipitation_6_Hour_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,precipitation_6_Hour,jrsjid1)
                        sql2 = "INSERT INTO tb_gdfs_precipitation_6_hour(gdfs_precipitation_6_Hour_time,gdfs_precipitation_6_Hour_lat,gdfs_precipitation_6_Hour_lon,gdfs_precipitation_6_Hour_value,gdfs_precipitation_6_Hour_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,precipitation_6_Hour,jrsjid2)
                        sql3 = "INSERT INTO tb_gdfs_precipitation_6_hour(gdfs_precipitation_6_Hour_time,gdfs_precipitation_6_Hour_lat,gdfs_precipitation_6_Hour_lon,gdfs_precipitation_6_Hour_value,gdfs_precipitation_6_Hour_jrsjid) \
                                VALUES ('%s','%s','%s','%s','%s')" % (time,lat,lon,precipitation_6_Hour,jrsjid3)

                        try:
                            cursor1.execute(sql1)
                            print('tb_gdfs_precipitation_6_Hour存入成功')
                            print('111111111111111111111111111111111111111111111111111111111111111111111111111')
                            # conn1.commit()
                        except Exception as e:
                            print("GDFSNcToJson_QPF_R06(1) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_QPF_R06(2) ----- ",e)
                        ...
                        try:
                            ...
                            print("GDFSNcToJson_QPF_R06(3) ----- ",e)

            try:
                cursor1.execute(sql4)
                # conn1.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_R06(4) ----- ")
                
            try:
                cursor2.execute(sql5)
                # conn2.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_R06(5) ----- ")
                
            try:
                cursor3.execute(sql6)
                # conn3.commit()
            except Exception as e:
                print("GDFSNcToJson_QPF_R06(6) ----- ")

            return jsondictList

        except Exception as e:
            print("GDFSNcToJson_QPF_R06 文件解析异常 ----- ")

    except Exception as e:
        print("GDFSNcToJson_QPF_R06 数据库连接异常 ----- ")
    finally:
        cursor1.close()
        conn1.close()
        cursor2.close()
