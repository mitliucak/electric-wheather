...
def SkzdzsDelete():
    try:
        ...
        sql3 = "DELETE FROM tb_skzdzs24_jx WHERE skzdzs_time < '%s' " % (
            (datetime.datetime.now() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d %H:%M:%S"))
        cursor3.execute(sql3)
        print("13308 tb_skzdzs24_jx表删除成功")
    except Exception as e:
        print("删除tb_skzdzs_jx24小时前记录失败")
    finally:
        cursor3.close()
        conn3.close()
        ...
        for line in file:
            try:
                ...
                sql_input = "INSERT INTO tb_skzdzs_jx (skzdzs_stationname,skzdzs_province,skzdzs_city,skzdzs_cnty," \
                            ...
                            "skzdzs_rhu,skzdzs_pre24,skzdzs_windavg10mi,skzdzs_winsavg10mi,skzdzs_windsmax,skzdzs_windinstmax,skzdzs_winsinstmax,skzdzs_clocov,skzdzs_stationcategory,skzdzs_jrsjid,skzdzs_count) " \
                            ...
                            % (lst[0],lst[1],lst[2],lst[3],lst[5],lst[6],lst[7],zdtime
                               ...
                               StationCategory,jrsjid,count)
                
                sql_input3 = "INSERT INTO tb_skzdzs_jx (skzdzs_stationname,skzdzs_province,skzdzs_city,skzdzs_cnty," \
                            "skzdzs_stationidd,skzdzs_lat,skzdzs_lon,skzdzs_time,skzdzs_prs,skzdzs_tem," \
                ...
                sql_input24 = "INSERT INTO tb_skzdzs24_jx (skzdzs_stationname,skzdzs_province,skzdzs_city,skzdzs_cnty," \
                            "skzdzs_stationidd,skzdzs_lat,skzdzs_lon,skzdzs_time,skzdzs_prs,skzdzs_tem," \
                            "skzdzs_rhu,skzdzs_pre24,skzdzs_windavg10mi,skzdzs_winsavg10mi,skzdzs_windsmax,skzdzs_windinstmax,skzdzs_winsinstmax,skzdzs_clocov,skzdzs_stationcategory,skzdzs_jrsjid) " \
                            "VALUES ('%s','%s','%s','%s',%s,%s,%s,'%s',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'%s',%s)" \
                            % (lst[0],lst[1],lst[2],lst[3],lst[5],lst[6],lst[7],zdtime
                ...
                conn3 = jaydebeapi.connect(driver,jdbc_url3,uandp,jar_file)
                cursor3 = conn3.cursor()
                # conn24 = jaydebeapi.connect(driver,jdbc_url3,uandp,jar_file)
                ...
                print(StationCategory,'实时数据存入成功',filepath)
                cursor2.execute(sql_input)
                cursor3.execute(sql_input3)
                cursor3.execute(sql_input24)
            except Exception as e:
                print("SkzdzsIntoMysql(2) -----继续 ",e)
                logstatus = 0
                type = "解析数据存储"
                failinfo = ""
                for each in e.args:
...
def AlarmDelete():
    try:
        conn3 = jaydebeapi.connect(driver,jdbc_url3,uandp,jar_file)
        cursor3 = conn3.cursor()
        sql3 = "DELETE FROM tb_alarm24 WHERE alarm_sendTime < '%s' " % (
            (datetime.datetime.now() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d %H:%M:%S"))
        
        cursor3.execute(sql3)
        print("13308 tb_alarm24表删除成功")
    except Exception as e:
        print("删除tb_alarm24小时前记录失败")
    finally:
        cursor3.close()
        conn3.close()
...
def AlarmJxIntoMysql(ZipFilePath,jrsjid):
    try:
        ...
        sql24 = "INSERT INTO tb_alarm24(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)" \
              "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s',%s)" \
              % (
              aKeys[0], aKeys[1], aKeys[2], aKeys[3], aKeys[4], aKeys[5], aKeys[6], aKeys[7], aKeys[8], aKeys[9], aKeys[10],
              aKeys[11], aKeys[12], aKeys[13], aKeys[14], aKeys[15], aKeys[16], aKeys[17], aKeys[18], aKeys[19], aKeys[20],
        ...
        try:
            ...
            time.sleep(1)
            print("tb_alarm3存入成功")
            cursor3.execute(sql24)
            print("---------------插入tb_alarm表格成功---------------")
        except Exception as e:
            print("---------------插入tb_alarm表格失败AlarmJxIntoMysql(2)--------------- ",e)
        finally:
            cursor1.close()
            conn1.close()
            cursor2.close()
        ...
        print("-------------没走插入tb_alarm表格    AlarmJxIntoMysql(1) ----- ",e)
    ...
    try:
        ...
        for line in file:
            try:
                ...
                sql_input24 = "INSERT INTO tb_cgqxzh24 (cgqxzh_stationname,cgqxzh_province,cgqxzh_city,cgqxzh_cnty," \
                            "cgqxzh_stationidd,cgqxzh_time,cgqxzh_lat,cgqxzh_lon,cgqxzh_prsavg,cgqxzh_wins2miavg," \
                            "cgqxzh_temavg,cgqxzh_temmax,cgqxzh_temmaxotime,cgqxzh_temmin,cgqxzh_temminotime,cgqxzh_rhuavg,cgqxzh_pre2020,cgqxzh_pre0808,cgqxzh_pre2008,cgqxzh_pre0820, cgqxzh_jrsjid) " \
                            "VALUES ('%s', '%s', '%s', '%s', %s,'%s',%s,%s, %s, %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" \
                            % (lst[0], lst[1], lst[3], lst[4], lst[6], zdtime, lst[11], lst[12], lst[13], lst[14]
    ...
    try:
        ...
        for line in file:
            try:
                ...
                # print(sql_input)
                sql_input24 = "INSERT INTO tb_hjqxzh24 (hjqxzh_stationname,hjqxzh_province,hjqxzh_city,hjqxzh_cnty," \
                            "hjqxzh_stationidd,hjqxzh_time,hjqxzh_lat,hjqxzh_lon,hjqxzh_prsavg," \
                            "hjqxzh_dew,hjqxzh_frost,hjqxzh_ice,hjqxzh_smoke,hjqxzh_haze,hjqxzh_fldu,hjqxzh_flduotime," \
                            "hjqxzh_flsa,hjqxzh_flsaotime,hjqxzh_duwhr,hjqxzh_mist,hjqxzh_lit,hjqxzh_aur,hjqxzh_gawin," \
                            "hjqxzh_gss,hjqxzh_thund,hjqxzh_squa,hjqxzh_tord,hjqxzh_sast,hjqxzh_sastotime,hjqxzh_drsnow," \
    ...
    try:
        ...
        for line in file:
            try:
                ...
                sql_input24 = "INSERT INTO tb_dxjbzh24 (dxjbzh_stationname,dxjbzh_province,dxjbzh_city,dxjbzh_cnty," \
                            "dxjbzh_stationidd,dxjbzh_time,dxjbzh_lat,dxjbzh_lon,dxjbzh_eice,dxjbzh_eicetns," \
                            "dxjbzh_eicetwe,dxjbzh_eicewns,dxjbzh_eicewwe,dxjbzh_eicedns,dxjbzh_eicedwe,dxjbzh_jrsjid) " \
                            "VALUES ('%s', '%s', '%s', '%s', %s,'%s',%s,%s, %s, %s,%s,%s,%s,%s,%s,%s)" \
                            % (lst[0], lst[1], lst[3], lst[4], lst[6], zdtime, lst[11], lst[12], lst[13], lst[14]
            ...
            try:
                ...
                for item in j1list:
                    ...
                    try:
                        ...
                        cursor2.execute(sql_input)
                        cursor3.execute(sql_input)
                        print('tb_ocf存入成功1111111111111111111111111111111111111111111111111111')
                    except Exception as e:
                        print("OCFIntoMysql(3) ----- ",e)
                    finally:
                        cursor1.close()
                        conn1.close()
                        cursor2.close()
