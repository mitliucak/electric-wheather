...
def NcToJson(filePath, ncdatadict, jrsjid1, jrsjid2, jrsjid3, AttrName):
    try:
            ...
            for i in range(TimeDimension):
                for j in range(LatDimension):
                    for k in range(LonDimension):
                        ...
                        sql = "INSERT INTO tb_gdfs_visibility_surface(gdfs_visibility_surface_time,gdfs_visibility_surface_lat,gdfs_visibility_surface_lon,gdfs_visibility_surface_value,gdfs_visibility_surface_jrsjid) \
                                VALUES (%s,%s,%s,%s,%s)"
                        arr = (time,lat,lon,visibility_surface,jrsjid1)
                        tup.append(arr)
                        if len(tup) == 1000:
                            try:
                                cursor.executemany(sql,tup)
                                conn.commit()
                                print('tb_GDFS_vis存入成功',filePath)
                            except Exception as e:
                                print('数据连接报错并且数据存入失败'+str(e))
                            tup = []
                        #@ try:
                            #@ cursor1.execute(sql1)
                            # conn1.commit()
                        #@ except Exception as e:
                            #@ print("GDFSNcToJson_SHYS_VIS(1) ----- ")

                        #@ try:
                            #@ cursor2.execute(sql2)
                            # conn2.commit()
                        #@ except Exception as e:
                            #@ print("GDFSNcToJson_SHYS_VIS(2) ----- ")

                        #@ try:
                            #@ cursor3.execute(sql3)
                            # conn3.commit()
                        #@ except Exception as e:
                            #@ print("GDFSNcToJson_SHYS_VIS(3) ----- ")
...
def main(filePath, jrsjid1, jrsjid2, jrsjid3):
    ...
    # Js24代表过去24小时降水
    jsondictList = NcToJson(filePath, ncdatadict, jrsjid1, jrsjid2, jrsjid3, 'Js24')
