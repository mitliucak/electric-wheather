...
# conn = pymysql.connect(host= 'localhost',db='06dlqxsync',user='root',password='root',charset='utf8',port=3306)
...
def ReadFromNc(filePath,jrsjid1,jrsjid2,jrsjid3):
    try:
        ...
        try:
                ...
                for i in range(m):
                    for j in range(n):
                        ...
                        sql2 = "INSERT INTO tb_laps(laps_xcoordinate,laps_ycoordinate,laps_ldtem,laps_xdsd,laps_tem,laps_jslyxs,laps_winddirect,laps_windspeed,laps_ucom,laps_vcom,laps_time,laps_jrsjid) \
                                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                        arr = (XCoordinate, YCoordinate, LDTem, Xdsd, Tem, JslYxs, WindDirect, WindSpeed, UCom, VCom, Time,jrsjid1)
                        tup.append(arr)
                        if len(tup)==1000:


                            try:
                                cursor.executemany(sql2,tup)
                             
                                conn.commit()
                                print("-----------d1数据库tb_laps表插入成功--------",filePath)
                            except Exception as e:
                                print("LAPSNcToJson(1) ----- "+str(e))
                            tup = []
                        # try:
                            # cursor.execute(sql1)
            ...
            print("LAPSNcToJson 文件解析异常 ----- "+str(e))

    except Exception as e:
        print("LAPSNcToJson 数据库连接异常 ----- "+str(e))
    finally:
        cursor.close()
        conn.close()
        # cursor1.close()
